...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            # ⚡ Bolt Optimization: Using math.exp instead of numpy.exp avoids Python-to-C
            # conversion overhead for single scalars and avoids inline import overhead.
            # math.exp raises OverflowError for large inputs; we handle it cleanly.
            try:
