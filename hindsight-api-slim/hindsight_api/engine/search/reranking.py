...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            try:
                # Use math.exp instead of numpy.exp for single scalar operations
                # to prevent Python-to-C conversion overhead in hot paths.
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # If -x is too large (i.e. x is highly negative), math.exp overflows,
                # which means the denominator approaches infinity, so the result is 0.0.
                return 0.0
