...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            try:
                # ⚡ Bolt: Use math.exp instead of np.exp to avoid inline import and Python-to-C context
                # switching overhead for single scalar operations.
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # math.exp throws on overflow (large positive inputs to -x), meaning exp(-x) is inf.
                return 0.0
