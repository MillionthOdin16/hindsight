...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            try:
                # Use math.exp instead of np.exp to avoid Python-to-C conversion overhead
                # for single scalar operations in this hot path.
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # math.exp raises OverflowError for large negative x, whereas np.exp returns inf.
                # 1 / (1 + inf) == 0.0
                return 0.0
