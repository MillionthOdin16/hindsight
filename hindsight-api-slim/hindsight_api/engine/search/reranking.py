...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            # ⚡ Bolt: Using math.exp instead of numpy.exp for scalar operations to avoid C-extension overhead.
            try:
                return 1.0 / (1.0 + math.exp(-x))
            except OverflowError:
                # x is large negative -> math.exp(-x) is too large -> sigmoid is 0.0
                return 0.0
