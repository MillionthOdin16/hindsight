...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            # Replaced numpy.exp with math.exp for scalar operations to avoid
            # Python-to-C overhead. math.exp raises OverflowError on large
            # negatives, whereas np.exp returned 0.0. Catching OverflowError
            # preserves np.exp behavior.
            try:
