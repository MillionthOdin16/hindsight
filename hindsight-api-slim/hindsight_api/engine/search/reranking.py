...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            # ⚡ Bolt: Use math.exp instead of numpy.exp for scalar operations to avoid
            # Python-to-C conversion and inline import overhead in this hot path.
            try:
                return 1.0 / (1.0 + math.exp(-x))
            except OverflowError:
                # math.exp raises OverflowError on large negative values (e.g. -800)
                # whereas numpy.exp returns 0.0. To match behavior and handle limits:
                return 0.0
