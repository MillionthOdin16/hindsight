...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        # Inline numpy import is slow and np.exp is slower than math.exp for scalars.
        def _sigmoid(x: float) -> float:
            try:
                # math.exp can overflow for large inputs (e.g. highly negative -x).
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # When x is highly negative, math.exp(-x) -> +inf, 1/(1+inf) -> 0.0.
                return 0.0
