...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            try:
                # Use math.exp instead of np.exp for scalar performance optimization
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # math.exp raises OverflowError for large inputs (e.g. highly negative x -> exp(large positive))
                # x is highly negative -> exp(-x) is large positive -> 1 / (1 + large) -> 0.0
                return 0.0
