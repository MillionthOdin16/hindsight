...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            # ⚡ Optimization: using math.exp instead of numpy.exp for scalar operations.
            # math.exp avoids the Python-to-C conversion overhead of NumPy in this hot loop,
            # which processes many search candidates per request.
            # math.exp raises OverflowError on large negative values (unlike np.exp which returns 0.0).
            try:
