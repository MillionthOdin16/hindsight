...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            # Inline import of heavy libraries like numpy in hot paths is a performance anti-pattern.
            # Using math.exp for scalar values is faster and avoids Python-to-C overhead.
            try:
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # math.exp raises OverflowError for large positive arguments (i.e. x is highly negative).
                # The sigmoid of a highly negative number approaches 0.
                return 0.0
