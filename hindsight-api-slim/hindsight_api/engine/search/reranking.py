...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        # Performance optimization: Replace numpy.exp with math.exp for scalar value processing.
        # math.exp is ~5x faster than numpy.exp on single scalar operations because it avoids
        # Python-to-C API overhead and type coercion required by numpy arrays.
        def _sigmoid(x: float) -> float:
            try:
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # Math.exp raises OverflowError on very large positive or negative values
                if x < 0:
                    return 0.0
                return 1.0
