...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        # Optimized to use math.exp to avoid Python-to-C conversion overhead for single scalar operations.
        def _sigmoid(x: float) -> float:
            try:
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # math.exp raises OverflowError for large inputs; fallback to 0.0 to match numpy.exp behavior for large negatives.
                return 0.0
