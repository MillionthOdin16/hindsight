...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            # Using math.exp instead of np.exp for single scalar operations
            # avoids ~7x Python-to-C conversion overhead.
            try:
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # math.exp(-x) overflows for large negative x, returning inf
                # so 1 / (1 + inf) approaches 0.0
                return 0.0
