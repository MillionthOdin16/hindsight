...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            try:
                # Use math.exp instead of numpy.exp for scalar operation performance.
                # math.exp raises OverflowError for large positive values (-x is very positive).
                return 1.0 / (1.0 + math.exp(-x))
            except OverflowError:
                # If -x is very large (i.e. x is very negative), math.exp(-x) overflows.
                # The denominator becomes infinite, so the sigmoid value is 0.0.
                return 0.0
