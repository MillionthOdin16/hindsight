...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            # ⚡ Bolt Optimization: Replacing numpy.exp with math.exp for scalar operations
            # in a hot path avoids Python-to-C conversion overhead.
            try:
                # math.exp raises OverflowError on large negative inputs, whereas np.exp returns 0.
                return 1 / (1 + math.exp(-x))
            except OverflowError:
                # If math.exp(-x) overflows, it means -x is very large and positive,
                # so the denominator tends to infinity and sigmoid is 0.
                return 0.0
