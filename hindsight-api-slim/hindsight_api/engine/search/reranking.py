...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            # ⚡ Bolt: Using math.exp instead of np.exp for scalar operation gives ~5x speedup
            # Wrapping in try-except to handle OverflowError (numpy handles this by returning 0 or inf)
            try:
