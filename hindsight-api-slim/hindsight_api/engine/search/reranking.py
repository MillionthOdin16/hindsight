...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        # ⚡ Bolt: replaced numpy.exp with math.exp for scalar operation performance.
        # This avoids the Python-to-C overhead of NumPy for single scalar values.
        def _sigmoid(x: float) -> float:
            try:
                return 1.0 / (1.0 + math.exp(-x))
            except OverflowError:
                # Math.exp raises OverflowError for highly positive -x (highly negative x).
                # The denominator becomes infinity, so the result is 0.0.
                return 0.0
