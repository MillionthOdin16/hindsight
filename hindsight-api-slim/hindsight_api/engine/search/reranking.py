...
class CrossEncoderReranker:
    ...
    async def rerank(self, query: str, candidates: list[MergedCandidate]) -> list[ScoredResult]:
        ...
        def _sigmoid(x: float) -> float:
            try:
                # Use math.exp instead of numpy.exp for scalar operations to avoid
                # Python-to-C conversion overhead.
                return 1.0 / (1.0 + math.exp(-x))
            except OverflowError:
                # math.exp raises OverflowError for large negative inputs (e.g. -x > 709).
                # This corresponds to exp(-x) being infinity, meaning the denominator
                # is infinite, so the sigmoid approaches 0.0.
                return 0.0
